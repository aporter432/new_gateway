...
- Production: Full retry mechanism, info/error logging, direct OGx connection

For retry policies, see OGx-1.txt section 3.3.
For error handling, see OGx-1.txt section 4.2.
"""
...
from Protexis_Command.api_ogx.constants import GatewayErrorCode
from Protexis_Command.api_ogx.constants.ogx_limits import DEFAULT_WINDOW_SECONDS
from Protexis_Command.api_ogx.services.ogx_message_queue import OGxMessageQueue
from Protexis_Command.api_ogx.services.ogx_message_submission import submit_OGx_message
from Protexis_Command.api_ogx.validation.ogx_validation_exceptions import OGxProtocolError
...
class MessageWorker:
    ...
    def __init__(self, settings: Settings, message_queue: OGxMessageQueue):
        """Initialize worker.
            ...
            try:
                ...
                for message in messages:
                    ...
                    try:
                        ...
                        # Submit to OGx
                        response = await submit_OGx_message(message.payload)

                        # Handle rate limiting using existing error code
                        if response.get("ErrorID") == GatewayErrorCode.SUBMIT_MESSAGE_RATE_EXCEEDED:
...
async def get_message_worker() -> MessageWorker:
    ...
    message_queue = OGxMessageQueue(redis, settings)
