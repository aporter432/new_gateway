...
This module implements message delivery following OGWS-1.txt specifications.
...
- OGWS-1.txt Section 5.1: Message Format
    - Strict validation in production
    - Flexible validation in development
- OGWS-1.txt Section 7.2: Error Recovery
...
Production:
    ...
    - Direct OGWS connection with TLS
...
class MessageSender:
    ...
    def __init__(self) -> None:
        ...
        self._rate_limiter: Dict[str, List[datetime]] = {}  # Track API call timestamps per group
        self._retry_counts: Dict[int, int] = {}  # Track retry attempts per message ID
        self.logger = get_protocol_logger("message_sender")
        self.settings = get_settings()
        self.auth_manager: Optional[OGWSAuthManager] = None  # Will be initialized in initialize()
    ...
    async def retry_failed_message(
        ...
        Implementation follows OGWS-1.txt Section 7.2 for retry handling:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.settings.OGWS_BASE_URL}/submit/messages/{message_id}/retry",
    ...
    def _can_make_request(self) -> bool:
        ...
        Implements sliding window rate limiting as specified in OGWS-1.txt.
