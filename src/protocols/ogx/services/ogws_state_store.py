...
It implements state management as defined in OGWS-1.txt Section 5.3, supporting:
...
Configuration Sources:
    ...
    - OGWS-1.txt Section 5.3: State transition rules
...
from protocols.ogx.exceptions import EncodingError, OGxProtocolError
...
class MessageStateStore(ABC):
    ...
    async def get_state(self, message_id: int) -> Optional[Dict]:
        ...
        Implements state retrieval as defined in OGWS-1.txt Section 5.3.
    ...
    async def get_state_history(self, message_id: int) -> List[Dict]:
        ...
        Implements state history tracking as defined in OGWS-1.txt Section 5.3.
...
class RedisMessageStateStore(MessageStateStore):
    ...
    async def update_state(
        ...
        try:
            ...
            self.logger.error(
                "Failed to update message %d state: %s",
                message_id,
                str(e),
                extra={
                    "message_id": message_id,
            ...
            raise OGxProtocolError(f"Failed to update message {message_id} state: {str(e)}") from e
